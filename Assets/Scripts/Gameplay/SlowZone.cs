...
{
    [Header("Slow Effect")]
    [Tooltip("Speed multiplier for enemies in this zone (0.15 = 15% speed = very slow, 0.5 = 50% speed = half speed)")]
    [Range(0.01f, 1f)]
    ...
    [Header("Debug")]
    [Tooltip("Show debug logs when enemies enter/exit")]
    ...
    private void ApplySlowEffect(Collider col, bool entering)
    {
        ...
        {
            if (debugLogs)
                Debug.Log($"[SlowZone] {(entering ? "ENTER" : "EXIT")}: {col.name} has BehaviorManager on {behaviorManager.gameObject.name}", this);
        ...
        {
            if (debugLogs)
                Debug.Log($"[SlowZone] {(entering ? "ENTER" : "EXIT")}: {col.name} has EnemyController on {enemyController.gameObject.name}", this);
        ...
        {
            if (debugLogs)
                Debug.Log($"[SlowZone] {(entering ? "ENTER" : "EXIT")}: {col.name} has MonsterAStarFollower on {monster.gameObject.name}", this);
            ApplyToEnemy(monster, entering);
            return;
        }

        if (debugLogs)
            Debug.LogWarning($"[SlowZone] {(entering ? "ENTER" : "EXIT")}: {col.name} has no recognized enemy component! GameObject: {col.gameObject.name}, Parent: {(col.transform.parent != null ? col.transform.parent.name : "none")}", this);
    }
    ...
    private void SetEnemySpeedMultiplier(MonoBehaviour enemy, float multiplier)
    {
        ...
        if (wander != null)
        {
            ...
            if (debugLogs)
                Debug.Log($"[SlowZone] Applied speed multiplier {multiplier} to WanderBehavior on {enemyObj.name}", this);
        }
        ...
        if (hunt != null)
        {
            ...
            foundAnyBehavior = true;
            if (debugLogs)
                Debug.Log($"[SlowZone] Applied speed multiplier {multiplier} to HuntBehavior on {enemyObj.name}", this);
        }
        ...
        if (attack != null)
        {
            ...
            foundAnyBehavior = true;
            if (debugLogs)
                Debug.Log($"[SlowZone] Applied speed multiplier {multiplier} to AttackBehavior on {enemyObj.name}", this);
        }
        ...
        if (travel != null)
        {
            ...
            foundAnyBehavior = true;
            if (debugLogs)
                Debug.Log($"[SlowZone] Applied speed multiplier {multiplier} to TravelBehavior on {enemyObj.name}", this);
        }
        ...
        if (followStroke != null)
        {
            ...
            foundAnyBehavior = true;
            if (debugLogs)
                Debug.Log($"[SlowZone] Applied speed multiplier {multiplier} to FollowStrokeBehavior on {enemyObj.name}", this);
        }
        ...
        if (controller != null)
        {
            ...
            foundAnyBehavior = true;
            if (debugLogs)
                Debug.Log($"[SlowZone] Applied speed multiplier {multiplier} to EnemyController on {enemyObj.name}", this);
        }
        ...
        if (monster != null)
        {
            ...
            foundAnyBehavior = true;
            if (debugLogs)
                Debug.Log($"[SlowZone] Applied speed multiplier {multiplier} to MonsterAStarFollower on {enemyObj.name}", this);
        }

        if (!foundAnyBehavior && debugLogs)
        {
            Debug.LogWarning($"[SlowZone] No speed-multiplier-compatible behavior found on {enemyObj.name}! Available components: {string.Join(", ", enemyObj.GetComponents<MonoBehaviour>().Select(c => c.GetType().Name))}", this);
        }
    }
