    ...
    namespace JellyGame.GamePlay.Painting.Shapes
    {
        ...
        public class StrokeTrailAnalyzer : MonoBehaviour, IMovementPainter
        {
            ...
            [Header("Debug")]
            [Tooltip("Log when loops are accepted, sent to detectors, or history is cleared.")]
            ...
            {
                // Re-arm detection for the next stroke.
                _closureHandled = false;
            }
            ...
            private void TryDetectLoopAndNotifyDetectors()
            {
                ...
                for (int i = 0; i < last - 1; i++)
                {
                    ...
                    if (indexDiff < minLoopIndexSeparation)
                        continue; // too small wiggle

                    float chord = Vector3.Distance(lastPos, _history[i].WorldPos);
                    if (chord < bestChord)
                    {
                        // still compute loop length just for debug
                ...
                // No candidate with enough index separation.
                if (bestStart < 0)
                {
                    if (debugLoopVerbose)
                ...
                // In both cases (shape or scribble) we now clear the path,
                // so any turns that happened before this loop are gone.
                _history.ConsumeUpTo(last);
                _closureHandled = false;  // allow new loops immediately.

                if (debugLoop)
                {
                    if (handledByAny)
                        Debug.Log("[StrokeTrailAnalyzer] Loop handled by a shape detector → history cleared.");
                    else
                        Debug.Log("[StrokeTrailAnalyzer] No detector recognized this loop → treated as scribble, history cleared.");
                }
            }

            private void OnValidate()
            {
                if (closureMaxDistance < 0f) closureMaxDistance = 0f;
                if (minLoopIndexSeparation < 1) minLoopIndexSeparation = 1;
                if (minCornerSeparationMeters < 0f) minCornerSeparationMeters = 0f;
            }
        }
    }
